...
namespace SpinAPI
{
	...
	bool State::ParseMz(const std::string& _mz, int& _out)
	{
		...
		catch(const std::exception&) {return false;}	// In case of exceptions from std::stoi
	...
	bool State::ParseFactor(const std::string& _factor, arma::cx_double& _out)
	{
		...
		catch(const std::exception&) {return false;}	// Catch exceptions from the uses of std::stod
