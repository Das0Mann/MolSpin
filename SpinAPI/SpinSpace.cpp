...
namespace SpinAPI
{
	...
	// -----------------------------------------------------
	SpinSpace::SpinSpace()	: useSuperspace(false), spins(), interactions(), transitions(), time(0.0), trajectoryStep(0), useTrajectoryStep(false),
								reactionOperators(ReactionOperatorType::Haberkorn)
	{
	}
	
	SpinSpace::SpinSpace(const spin_ptr& _spin)	: useSuperspace(false), spins(), interactions(), transitions(), time(0.0), trajectoryStep(0),
													useTrajectoryStep(false), reactionOperators(ReactionOperatorType::Haberkorn)
	{
			this->spins.push_back(_spin);
	}
	
	SpinSpace::SpinSpace(const std::vector<spin_ptr>& _spinlist)	: useSuperspace(false), spins(_spinlist), interactions(), transitions(), time(0.0),
																		trajectoryStep(0), useTrajectoryStep(false), reactionOperators(ReactionOperatorType::Haberkorn)
	{
	}
	
	SpinSpace::SpinSpace(const std::shared_ptr<SpinSystem>& _system)	: useSuperspace(false), spins(), interactions(), transitions(), time(0.0),
																			trajectoryStep(0), useTrajectoryStep(false), reactionOperators(ReactionOperatorType::Haberkorn)
	{
		this->Add(_system);
	}
	
	SpinSpace::SpinSpace(const SpinSystem& _system)	: useSuperspace(false), spins(_system.Spins()), interactions(_system.Interactions()), transitions(_system.Transitions()),
	...
	SpinSpace::SpinSpace(const SpinSpace& _space)	: useSuperspace(_space.useSuperspace), spins(_space.spins), interactions(_space.interactions), transitions(_space.transitions),
