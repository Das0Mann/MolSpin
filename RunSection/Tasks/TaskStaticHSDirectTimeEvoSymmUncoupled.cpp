...
namespace RunSection
{
	...
	// -----------------------------------------------------
	TaskStaticHSDirectTimeEvoSymmUncoupled::TaskStaticHSDirectTimeEvoSymmUncoupled(const MSDParser::ObjectParser& _parser, const RunSection& _runsection)	: BasicTask(_parser,_runsection), reactionOperators(SpinAPI::ReactionOperatorType::Haberkorn),
		...
		for(auto i = systems.cbegin(); i != systems.cend(); i++) // iteration through all spin systems, in this case (or usually), this is one
		{
			...
			if (InitialState == "singlet")
				...
				arma::cx_mat result(1,1);
                        	...
                        	arma::cx_mat B1t(2*Z1, 2*Z1); arma::cx_mat B2t(2*Z2, 2*Z2);	
				...
				for(int indx = 0; indx < num_steps; indx++)
					...
					// Update B using the Higham propagator
                        ...
                        else if (InitialState == "tripletplus")
                        {
                                ...
                                arma::cx_mat B1t(Z1, 2*Z1); arma::cx_mat B2t(Z2, 2*Z2);
