	...
	bool TaskStaticSSRelaxation::RunLocal()
	{
		...
		for (auto i = systems.cbegin(); i != systems.cend(); i++)
		{
			...
			for (auto j = this->relaxingSpins.cbegin(); j != this->relaxingSpins.cend(); j++)
			{
				...
				arma::sp_cx_mat SSy3;
				arma::sp_cx_mat SSz3;
				if (!space.SuperoperatorFromOperators(Sx, Sx, SSx1) || !space.SuperoperatorFromOperators(Sy, Sy, SSy1) || !space.SuperoperatorFromOperators(Sz, Sz, SSz1) || !space.SuperoperatorFromLeftOperator(Sx * Sx, SSx2) || !space.SuperoperatorFromLeftOperator(Sy * Sy, SSy2) || !space.SuperoperatorFromLeftOperator(Sz * Sz, SSz2) || !space.SuperoperatorFromRightOperator(Sx * Sx, SSx3) || !space.SuperoperatorFromRightOperator(Sy * Sy, SSy3) || !space.SuperoperatorFromRightOperator(Sz * Sz, SSz3))
				{
					this->Log() << "Failed to obtain spin operators for spin \"" << (*j) << "\"." << std::endl;
					continue;
				}
