...
#pragma omp parallel for firstprivate(ptr_Tensors) shared(eigen_vec) num_threads(threads)
									...
									delete[] ptr_SpecDens;
									ptr_SpecDens = new arma::sp_cx_mat *[num_element];
												...
												else
												{
													for (int n = 0; n < (int)ampl_mat.n_cols; n++)
													{
														...
														if ((*interaction)->Properties()->Get("def_specdens", def_specdens) && def_specdens == 1)
															...
															if (!ConstructSpecDensSpecificSparse(1, static_cast<std::complex<double>>(ampl_mat(m, n)), static_cast<std::complex<double>>(tau_c_mat(m, n)), domega, SpecDens))
															{
														...
														else
														{
															...
															if (!ConstructSpecDensSpecificSparse(0, static_cast<std::complex<double>>(ampl_mat(m, n)), static_cast<std::complex<double>>(tau_c_mat(m, n)), domega, SpecDens))
															{
																this->Log() << "There are problems with the construction of the spectral density matrix - Please check your input." << std::endl;
																continue;
															}
													...
													if (!RedfieldtensorSparse((*ptr_Tensors[k]), (*ptr_Tensors[s]), *ptr_SpecDens[m], tmp_R))
													{
									...
									else if ((*interaction)->Properties()->Get("terms", terms) && terms == 1 && ops == 1)
									{
										...
										for (k = 0; k < num_op; k++)
										{
											...
											else
											{
												...
												if (!RedfieldtensorSparse((*ptr_Tensors[k]), (*ptr_Tensors[k]), *ptr_SpecDens[m], tmp_R))
												{
													this->Log() << "There are problems with the construction of the Redfield tensor - Please check your input." << std::endl;
													continue;
												}
...
#pragma omp parallel for firstprivate(ptr_Tensors) shared(eigen_vec) num_threads(threads)
								...
								if ((*interaction)->Properties()->Get("slip", slip) && slip == 1)
								{
									...
									if (!SlippageSparse(ptr_Tensors, num_op, eig_val_mat, domega, rho0, static_cast<std::complex<double>>(tau_c_list[0]), rho0_new))
									{
...
#pragma omp parallel for firstprivate(tmp_R, SpecDens, ampl_combined) shared(ampl_list, tau_c_list, domega, num_op, ptr_Tensors, ptr_R, interaction, def_specdens) num_threads(threads)
										for (k = 0; k < num_op; k++)
										{
											...
											if ((*interaction)->Properties()->Get("def_specdens", def_specdens) && def_specdens == 1)
											{
												...
												if (!ConstructSpecDensSpecificSparse(1, static_cast<std::complex<double>>(ampl_combined), static_cast<std::complex<double>>(tau_c_list[0]), domega, SpecDens))
												{
													this->Log() << "There are problems with the construction of the spectral density matrix - Please check your input." << std::endl;
													continue;
												}
											...
											else
											{
												...
												if (!ConstructSpecDensSpecificSparse(0, ampl_combined, static_cast<std::complex<double>>(tau_c_list[0]), domega, SpecDens))
												{
													this->Log() << "There are problems with the construction of the spectral density matrix - Please check your input." << std::endl;
													continue;
												}
...
#pragma omp parallel for collapse(2) firstprivate(tmp_R, SpecDens, ampl_combined) shared(ampl_list, tau_c_list, domega, num_op, ptr_Tensors, ptr_R, interaction, def_specdens) num_threads(threads)
										for (k = 0; k < num_op; k++)
										{
											for (s = 0; s < num_op; s++)
											{
												...
												{
													SpecDens *= 0.0;
													if (!ConstructSpecDensSpecificSparse(1, ampl_combined, static_cast<std::complex<double>>(tau_c_list[0]), domega, SpecDens))
													{
														this->Log() << "There are problems with the construction of the spectral density matrix - Please check your input." << std::endl;
														continue;
													}
...
#pragma omp for
	...
	bool TaskStaticSSRedfieldSparse::SlippageSparse(arma::sp_cx_mat **_ptr_Tensors, const int &_num_op, const arma::sp_cx_mat &_eig_val_mat, const arma::sp_cx_mat &_domega, const arma::sp_cx_mat &_rho0, const std::complex<double> &_tau_c, arma::sp_cx_mat &_rho0_new)
	{
