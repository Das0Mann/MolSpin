										...
										for (k = 0; k < num_op; k++)
										{
											for (s = 0; s < num_op; s++)
											{
												...
												else
												{
													...
													if (!RedfieldtensorTimeEvo((*ptr_Tensors[k]), (*ptr_Tensors[s]), *ptr_SpecDens[m], tmp_R))
													{
													...
													if (k != s)
													{
														...
														if (!RedfieldtensorTimeEvo((*ptr_Tensors[s]), (*ptr_Tensors[k]), *ptr_SpecDens[m], tmp_R))
														{
															this->Log() << "There are problems with the construction of the Redfield tensor - Please check your input." << std::endl;
															continue;
														}
									...
									else if ((*interaction)->Properties()->Get("terms", terms) && terms == 1 && ops == 1)
									{
										...
										for (k = 0; k < num_op; k++)
										{
											...
											else
											{
												...
												if (!RedfieldtensorTimeEvo((*ptr_Tensors[k]), (*ptr_Tensors[k]), *ptr_SpecDens[m], tmp_R))
												{
													this->Log() << "There are problems with the construction of the Redfield tensor - Please check your input." << std::endl;
													continue;
												}
...
#pragma omp for
	...
	bool TaskStaticSSRedfieldTimeEvo::Slippage(arma::cx_mat **_ptr_Tensors, const int &_num_op, const arma::cx_mat &_eig_val_mat, const arma::cx_mat &_domega, const arma::cx_mat &_rho0, const std::complex<double> &_tau_c, arma::cx_mat &_rho0_new)
	{
		...
		for (int alpha = 0; alpha < _num_op; alpha++)
		{
			...
			// For T_alpha(k=1) Hermitian conjugate
			slip_op += _rho0.t() * (Tk1[alpha]).t() * (*_ptr_Tensors[alpha]).t() - (*_ptr_Tensors[alpha]).t() * _rho0.t() * (Tk1[alpha]).t();

			// For T_alpha(k=2) Hermitian conjugate, using .t() for conjugate transpose
			arma::cx_double i_complex(0.0, -1.0); // Use -i for the conjugate of i
			slip_op += (*_ptr_Tensors[alpha]).t() * (Tk2[alpha]).t() * _rho0.t() * i_complex * _eig_val_mat.t() - (Tk2[alpha]).t() * _rho0.t() * i_complex * _eig_val_mat.t() * (*_ptr_Tensors[alpha]).t();
		...
		//_rho0_new /= arma::trace(_rho0_new);
